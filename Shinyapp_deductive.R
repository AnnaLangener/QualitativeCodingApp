...
if(!file.exists(paste(Projectwd,User,"/Act_",ppID,".csv",sep = ""))){
  Empty <- data.frame(Code_Thought = rep(NA,nrow(Act_participant)), 
...
if(Levels_Thought == TRUE){
  ui <- navbarPage("Qualitative Coding",
                   ...
                   tabPanel("Thought, activity, location, and company",id = "Week",
                            tags$div(
...
server <- function(input, output, session){
  ...
  observeEvent(input$Act_participant_rows_current, {
    ...
    for (i in 1:nrow(Act_participant)) {
      subs_widget_Thought <- substitute({selectizeInput(paste0("selectize_code_Thought",i), NULL, choices=as.list(Codebook_Act_Thought), selected = c(unlist(str_split(Act[i,1]," ; "))), multiple = T,
    ...
    for (i in 1:nrow(Act_participant)) {
      subs_widget_Activity <- substitute({selectizeInput(paste0("selectize_code_Activity",i), NULL, choices=as.list(Codebook_Act_Activity), selected = c(unlist(str_split(Act[i,2]," ; "))), multiple = T,
                                                        options = list(render = I("
                                                      {
                                                        item: function(item, escape) { return '<div>' + item.label + '</div>'; },
                                                        option: function(item, escape) { return '<div>' + item.label + '</div>'; }
    ...
    for (i in 1:nrow(Act_participant)) {
      subs_widget_Location <- substitute({selectizeInput(paste0("selectize_code_Location",i), NULL, choices=as.list(Codebook_Act_Location), selected = c(unlist(str_split(Act[i,3]," ; "))), multiple = T,
                                                         options = list(render = I("
                                                      {
                                                        item: function(item, escape) { return '<div>' + item.label + '</div>'; },
                                                        option: function(item, escape) { return '<div>' + item.label + '</div>'; }
    ...
    for (i in 1:nrow(Act_participant)) {
      subs_widget_Company <- substitute({selectizeInput(paste0("selectize_code_Company",i), NULL, choices=as.list(Codebook_Act_Company), selected = c(unlist(str_split(Act[i,4]," ; "))), multiple = T,
                                                         options = list(render = I("
                                                      {
                                                        item: function(item, escape) { return '<div>' + item.label + '</div>'; },
                                                        option: function(item, escape) { return '<div>' + item.label + '</div>'; }
